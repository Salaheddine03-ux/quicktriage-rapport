...
d'apprentissage, le backend J2EE exposant le modèle, et le frontend mobile Android. Une
...
\begin{enumerate}
  ...
  \textbf{JAX-RS}.
  \item \textbf{La couche frontend (\textit{client mobile}).} Réalisée sous \textbf{Android}
  (Android Studio), elle assure la saisie des constantes vitales, l'appel à l'API et la
  restitution visuelle du résultat au personnel soignant.
\end{enumerate}
...
saisit les constantes dans l'application Android ; celles-ci sont sérialisées en JSON et
...
Android (Android Studio), Java/Kotlin, Retrofit/HTTP \\
\bottomrule
\end{tabular}
